...
public class MonthlyQuestTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        List<Quest> preparedQuestList = questRepositoryInterface.findQuestsByIsActiveAndQuestTypeOrderByQuestFrequencyAsc(false, 2);
        List<Quest> activatedQuestList = questRepositoryInterface.findQuestsByIsActiveAndQuestTypeOrderByQuestFrequencyAsc(true, 2);
        for(int i=0; i < 2 ; i++) {
